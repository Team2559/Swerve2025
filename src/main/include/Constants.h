...
constexpr units::unit_t<
    units::compound_unit<units::turns_per_second, units::inverse<units::volt>>>
    kVNeoVortex = 6784_rpm / 12.0_V;
} // namespace MotorConstants
...
inline constexpr double kDriveGearRatio =
    50.0 / 16.0 * 17.0 / 27.0 * 45.0 / 15.0; // approx 5.90
...
inline constexpr units::unit_t<
    units::compound_unit<units::meter, units::inverse<units::turn>>>
    kDriveDistancePerRotation =
...
constexpr units::meters_per_second_t kMaxDriveSpeed =
    6895_rpm * kDriveDistancePerRotation;
...
const units::meter_t kDriveMetersPerSteerCircle =
    M_PI * units::math::sqrt(units::math::pow<2>(kWheelbaseLength) +
                             units::math::pow<2>(kWheelbaseWidth));
...
constexpr double kV =
    (1.0 / MotorConstants::kVNeoVortex / kDriveDistancePerRotation).value();
} // namespace DrivePID
...
// AprilTag field data; FMA events should all use the welded field, but
// off-season events may use the AndyMark field instead.
const frc::AprilTagFieldLayout kAprilTags = frc::AprilTagFieldLayout::LoadField(
    frc::AprilTagField::k2025ReefscapeWelded);
// Camera focal point position and orientation relative to the robot origin
constexpr frc::Transform3d kRobotToCam =
    frc::Transform3d(frc::Translation3d(0.5_m, 0.0_m, 0.5_m),
