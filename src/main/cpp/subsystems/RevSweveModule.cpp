...
RevSwerveModule::RevSwerveModule(int driveCanID, int steerCanID,
                                 units::angle::turn_t offset)
      ...
      steerEncoder{steerMotor.GetAbsoluteEncoder()} {
  {
    ...
    driveConfig.SetIdleMode(SparkFlexConfig::IdleMode::kBrake)
  ...
  }
  {
    ...
    steerConfig.SetIdleMode(SparkFlexConfig::IdleMode::kCoast)
        .SmartCurrentLimit(60.0)
        .Inverted(kSteerMotorInverted);

    steerConfig.encoder
        .PositionConversionFactor(kSteerFeedbackScale / kSteerGearRatio)
        .VelocityConversionFactor(kSteerFeedbackScale / kSteerGearRatio *
                                  (1.0_s / 1.0_min));
    ...
    steerConfig.closedLoop
        ...
        .PositionWrappingInputRange(-0.5 * kSteerFeedbackScale,
                                    0.5 * kSteerFeedbackScale)
...
void RevSwerveModule::TestInit(std::string name) {
  ...
  nt_driveOutput = driveSetupTab.Add(name, std::array{0.0, 0.0, 0.0})
                       .WithWidget(frc::BuiltInWidgets::kGraph)
                       .GetEntry();
  nt_steerOutput = steerSetupTab.Add(name, std::array{0.0, 0.0, 0.0})
                       .WithWidget(frc::BuiltInWidgets::kGraph)
                       .GetEntry();
}
...
void RevSwerveModule::UpdateDrivePID(PIDUpdate &update) {
  ...
  driveMotor.Configure(config, SparkFlex::ResetMode::kNoResetSafeParameters,
                       SparkFlex::PersistMode::kNoPersistParameters);
}
...
void RevSwerveModule::UpdateSteerPID(PIDUpdate &update) {
  SparkFlexConfig config;
  BuildPIDConfig(config, update);
  steerMotor.Configure(config, SparkFlex::ResetMode::kNoResetSafeParameters,
                       SparkFlex::PersistMode::kNoPersistParameters);
}
...
units::angle::turn_t RevSwerveModule::GetSteerPosition() {
  return units::angle::turn_t{steerEncoder.GetPosition() *
                              kInvSteerFeedbackScale};
}

void RevSwerveModule::SetSteerPosition(units::angle::turn_t position) {
  steerMotor.GetClosedLoopController().SetReference(
      position.value() * kSteerFeedbackScale,
      SparkFlex::ControlType::kPosition);
}

void RevSwerveModule::StopSteer() { steerMotor.Set(0.0); }

void RevSwerveModule::SetDriveVelocity(
    units::velocity::meters_per_second_t velocity) {
  driveMotor.GetClosedLoopController().SetReference(
      velocity.value(), SparkFlex::ControlType::kVelocity, {},
      velocity.value() * driveVff);

  if (nt_driveOutput.has_value()) {
    nt_driveOutput.value()->SetDoubleArray(
        std::array{velocity.value(), driveEncoder.GetVelocity(),
                   driveMotor.GetAppliedOutput()});
  }
}

void RevSwerveModule::ResetDriveEncoder() { driveEncoder.SetPosition(0.0); }
...
void RevSwerveModule::SetDrivePercent(double percent) {
  ...
  if (nt_driveOutput.has_value()) {
    nt_driveOutput.value()->SetDoubleArray(std::array{
        0.0, driveEncoder.GetVelocity(), driveMotor.GetAppliedOutput()});
  }
}
...
const frc::SwerveModuleState RevSwerveModule::GetState() {
  return {units::velocity::meters_per_second_t{driveEncoder.GetVelocity()},
          GetSteerPosition()};
}

const frc::SwerveModulePosition RevSwerveModule::GetPosition() {
  return {units::length::meter_t{driveEncoder.GetPosition()},
          GetSteerPosition()};
}

void RevSwerveModule::SetDesiredState(frc::SwerveModuleState &state) {
  auto currentAngle = frc::Rotation2d(GetSteerPosition());
  // Allow modules to flip their "positive" direction when rapidly changing
