...
VisionSubsystem::VisionSubsystem(
    ...
    : SubsystemBase("Vision Subsystem"),
      ...
      m_poseEstimator{kAprilTags, photon::MULTI_TAG_PNP_ON_COPROCESSOR,
                      kRobotToCam},
      m_accessor{accessor}, m_updater{updater} {
  m_poseEstimator.SetMultiTagFallbackStrategy(kFallbackStrategy);
}

std::optional<frc::Pose3d> VisionSubsystem::SeedPose() {
  // Switch to a fallback strategy that doesn't depend on knowing an approximate
  // pose
  ...
  ProcessCameraResults(
      [&seed](frc::Pose3d pose, units::millisecond_t timestamp) {
        seed = pose;
      });
...
void VisionSubsystem::ProcessCameraResults(
    std::function<void(frc::Pose3d, units::millisecond_t)> updater) {
