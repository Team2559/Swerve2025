...
DriveSubsystem::DriveSubsystem()
    : SubsystemBase("Drive Subsystem"),
      frontLeftModule{new RevSwerveModule(kFrontLeftDriveMotorCanID,
                                          kFrontLeftSteerMotorCanID,
                                          kFrontLeftSteerOffset)},
      frontRightModule{new RevSwerveModule(kFrontRightDriveMotorCanID,
                                           kFrontRightSteerMotorCanID,
                                           kFrontRightSteerOffset)},
      rearLeftModule{new RevSwerveModule(kRearLeftDriveMotorCanID,
                                         kRearLeftSteerMotorCanID,
                                         kRearLeftSteerOffset)},
      rearRightModule{new RevSwerveModule(kRearRightDriveMotorCanID,
                                          kRearRightSteerMotorCanID,
                                          kRearRightSteerOffset)},
  ...
  frc::ShuffleboardLayout &xLayout =
      tab.GetLayout("X", frc::BuiltInLayouts::kList);
  nt_xPosition =
      xLayout.Add("Position [m]", initialPose.X().value()).GetEntry();
  nt_xSetpoint = xLayout.Add("Setpoint [m]", 0.0).GetEntry();
  nt_xOutput = xLayout.Add("Output [mps]", 0.0).GetEntry();

  frc::ShuffleboardLayout &yLayout =
      tab.GetLayout("Y", frc::BuiltInLayouts::kList);
  nt_yPosition =
      yLayout.Add("Position [m]", initialPose.Y().value()).GetEntry();
  nt_ySetpoint = yLayout.Add("Setpoint [m]", 0.0).GetEntry();
  nt_yOutput = yLayout.Add("Output [mps]", 0.0).GetEntry();

  frc::ShuffleboardLayout &rLayout =
      tab.GetLayout("R", frc::BuiltInLayouts::kList);
  nt_rPosition =
      rLayout
          .Add("Orientation [rad]",
               initialPose.Rotation().ToRotation2d().Radians().value())
          .GetEntry();
  ...
  m_poseEstimator = std::make_unique<frc::SwerveDrivePoseEstimator3d<4>>(
      kDriveKinematics, m_ahrs->GetRotation3d(), GetModulePositions(),
      initialPose);
...
void DriveSubsystem::Periodic() {
  ...
  frc::SmartDashboard::PutNumber(
      "Front left drive", frontLeftModule->GetPosition().distance.value());
  frc::SmartDashboard::PutNumber(
      "Front right drive", frontRightModule->GetPosition().distance.value());
  frc::SmartDashboard::PutNumber(
      "Rear left drive", rearLeftModule->GetPosition().distance.value());
  frc::SmartDashboard::PutNumber(
      "Rear right drive", rearRightModule->GetPosition().distance.value());

  frc::SmartDashboard::PutNumber(
      "Front left steer",
      frontLeftModule->GetSteerPosition().convert<units::deg>().value());
  frc::SmartDashboard::PutNumber(
      "Front right steer",
      frontRightModule->GetSteerPosition().convert<units::deg>().value());
  frc::SmartDashboard::PutNumber(
      "Rear left steer",
      rearLeftModule->GetSteerPosition().convert<units::deg>().value());
  frc::SmartDashboard::PutNumber(
      "Rear right steer",
      rearRightModule->GetSteerPosition().convert<units::deg>().value());
...
void DriveSubsystem::TestInit() {
  ...
  frc::ShuffleboardTab &driveSetupTab =
      frc::Shuffleboard::GetTab("Drive Setup");
  frc::ShuffleboardTab &steerSetupTab =
      frc::Shuffleboard::GetTab("Steer Setup");

  driveSetupTab.Add("PID", m_driveTuner)
      .WithWidget(frc::BuiltInWidgets::kPIDController);
  steerSetupTab.Add("PID", m_steerTuner)
      .WithWidget(frc::BuiltInWidgets::kPIDController);

  driveSetupTab.Add("Legend",
                    "(Blue) setpoint, (Red) measured, (Green) output");
  steerSetupTab.Add("Legend",
                    "(Blue) setpoint, (Red) measured, (Green) output");
...
void DriveSubsystem::ResetFieldOrientation(bool inverted) {
  m_poseEstimator->ResetPose(
      frc::Pose3d(GetPose().Translation(),
                  inverted ? frc::Rotation3d(frc::Rotation2d(180_deg))
                           : frc::Rotation3d()));
}
...
void DriveSubsystem::FollowTrajectory(const choreo::SwerveSample &sample) {
  ...
  units::radian_t target_heading =
      frc::AngleModulus(sample_heading - robot_heading) + robot_heading;
  ...
  Drive(sample.vx + xFeedback, sample.vy + yFeedback,
        sample.omega + rotFeedback, true);
}
...
const std::array<frc::SwerveModulePosition, 4>
DriveSubsystem::GetModulePositions() {
  return {frontLeftModule->GetPosition(), frontRightModule->GetPosition(),
          rearLeftModule->GetPosition(), rearRightModule->GetPosition()};
}

void DriveSubsystem::SetModuleStates(
    std::array<frc::SwerveModuleState, 4> desiredStates, bool steerOnly) {
  auto [frontLeft, frontRight, rearLeft, rearRight] = desiredStates;

  if (frontLeft.speed == 0.0_mps && frontRight.speed == 0.0_mps &&
      rearLeft.speed == 0.0_mps && rearRight.speed == 0.0_mps) {
    frontLeftModule->Stop();
    frontRightModule->Stop();
    rearLeftModule->Stop();
    rearRightModule->Stop();
...
void DriveSubsystem::UpdateVisionPose(frc::Pose3d measurement,
